...
- **좌표 고정**: 크롭은 녹화 시작 시점 화면 좌표로 고정한다. 스크롤해도 영역은 그대로다. 요소 녹화는 요소 따라가기(9.6절)를 켜면 요소 위치를 따라간다.
...
### 9.6 요소 따라가기 (`content/tracker.ts`, `core/crop.ts` trackedDraw)

요소 녹화 선택 패널의 **요소 따라가기** 토글(기본 켬, 마지막 선택을 `record.followElement`에 기억)을 켜면 녹화 중 요소의 화면 위치를 따라가며 자른다. 끄면 9.2절의 고정 좌표로 녹화한다.

```text
CS select:done(follow) → SW beginRecording → CS track:start(지금 위치 반환, 추적 시작)
→ OS rec:start({crop, track}) → cropTrack(track 모드)
CS: 프레임마다 요소 위치 측정 → 달라지면 OS로 rec:track(뷰포트 비율, 자르지 않은 값) — 서비스 워커를 거치지 않는다
OS: 프레임마다 최신 위치로 trackedDraw → OffscreenCanvas(고정 크기)에 그림 → new VideoFrame(canvas)
```

- **측정 시점**: 스크롤·리사이즈·애니메이션·레이아웃 변화를 한 경로로 잡으려고 이벤트 대신 `requestAnimationFrame`마다 `visibleRectOf`로 잰다. 값이 달라졌을 때만 보내고, 놓친 메시지에 대비해 500ms마다 다시 보낸다. rAF 안에서 재면 같은 프레임의 다른 rAF 콜백(페이지 스크립트의 스크롤)이 뒤이어 바꾼 위치를 놓쳐 한 프레임 늦는다. 그래서 rAF에서는 녹화 중 표시만 옮기고, 오프스크린에 보내는 값은 그린 직후(`MessageChannel`) 다시 잰다.
- **출력 크기 고정**: 첫 프레임의 요소 크기(화면보다 크면 화면 크기)로 캔버스를 만들고 끝까지 유지한다. `VideoFrame`의 `visibleRect`로는 크기를 고정할 수 없어 추적 모드만 캔버스에 그린다(오프스크린 문서에서도 `OffscreenCanvas` 2D는 rAF 없이 동작).
- **크기 변화(레터박스)**: 요소가 커지면 비율을 지켜 줄여 가운데에 두고, 작아지면 확대하지 않고 가운데에 둔다. 남는 곳은 검은색.
- **화면 밖 정책**: 일부만 보이면 보이는 부분을 요소 안 제자리에 그리고 나머지는 검은색. 전혀 보이지 않거나 문서에서 빠지면 **직전 화면을 유지**한다(빈 프레임은 깜빡임으로 보여 택하지 않음). 화면보다 큰 요소는 그 축에서 화면에 보이는 구간을 그대로 찍는다.
- **지연·흔들림 측정**: 240×120 요소를 프레임당 2·6·12px(60Hz 기준 120·360·720px/s)로 왕복 스크롤하며 결과 영상의 요소 경계 어긋남을 프레임마다 쟀다. rAF에서만 잴 때는 스크롤 속도만큼(최대 14px, 한 프레임 지연) 어긋났고, 그린 직후 측정으로 바꾼 뒤에는 모든 속도에서 정지 상태와 같은 2px 이내(인코더의 가장자리 색 번짐)였다. 보간·예측은 넣지 않는다.
- **녹화 중 표시**: 테두리는 요소를 따라가되, 합성 스레드 스크롤이 테두리 위치 갱신보다 먼저 화면에 나올 수 있어 움직이는 동안에는 숨기고 멈춘 뒤 250ms에 다시 보인다. 위젯은 요소와 겹치게 되면 빈 모서리로 옮긴다.
- **중지 조건**: 뷰포트 크기 변경·페이지 이동은 고정 좌표 녹화와 같이 `layout-changed`로 끝낸다. 탭 캡처 프레임이 고정 크기로 맞춰져 오는 동안 비율 기준이 달라지기 때문이다.
- **정리**: 녹화 종료·취소 때 서비스 워커가 `track:stop`을 보낸다. 받지 못해도 전송이 연속 실패하면 스스로 멈춘다.

**대안 비교: `getDisplayMedia` + Region Capture(`CropTarget.fromElement`)**

| | 채택: tabCapture + 위치 메시지 | getDisplayMedia + CropTarget |
| --- | --- | --- |
| 추적 정확도 | 메시지 1회 지연. 측정상 2px 이내 | 브라우저가 합성 단계에서 잘라 프레임과 정확히 일치 |
| 사용자 절차 | 추가 절차 없음(확장 아이콘·단축키 호출로 충분) | 녹화마다 화면 공유 선택 창. `preferCurrentTab`으로 줄여도 확인 클릭 필요 |
| 실행 위치 | 오프스크린 문서가 스트림 보유. 페이지 이동·서비스 워커 휴면과 무관 | `cropTo`는 캡처를 시작한 문서의 트랙에만 적용 가능 → 대상 페이지에서 스트림·MediaRecorder를 돌려야 하고 페이지 CSP·이동에 취약 |
| 출력 크기 | 고정(레터박스) | 요소 크기를 따라 프레임 크기가 변함 → MediaRecorder가 해상도 변경을 안정적으로 처리하지 못해 결국 캔버스 재합성 필요 |
| 가려짐 | 요소 위를 덮는 다른 요소도 함께 찍힘 | 같음(Region Capture도 위치만 자름. 가려짐 제거는 Element Capture `RestrictionTarget`) |
| 다른 모드와의 일관성 | 탭·영역 녹화와 같은 파이프라인 | 요소 녹화만 다른 권한 흐름과 코드 경로 |

정확도는 대안이 앞서지만 측정된 오차가 작고, 공유 창 없이 시작하는 흐름과 단일 파이프라인을 유지하는 쪽을 택했다.
...
type Settings = {
  ...
  record: {
    ...
    followElement: boolean;                                           // 기본 true. 선택 패널 토글이 기억(설정 화면에는 없음)
  };
...
| 녹화 크롭 좌표 | 영역은 시작 시점 고정, 요소는 따라가기 선택(기본 켬) | 스크롤하며 요소 변화를 기록하는 용도. 방식 비교는 9.6절 |
...
7. **후속**: GIF 변환, 내부 스크롤러 스티칭, iframe 내부 요소
